...
from suppliers.models import Package, SupplierSetting
...
class BookingSupplierPackageQuerySerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not Package.objects.filter(
            ...
            raise serializers.ValidationError(
                {'package_id': 'Invalid package for supplier.'},
            )
        return attrs
...
class BookingSupplierPackageView(APIView):
    ...
    def get(self, request):
        ...
        package_id = query.validated_data['package_id']
        ...
        package_name = package.package.name if package.package_id else ''
        return Response(
            {
                'id': package.id,
