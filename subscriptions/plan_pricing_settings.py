"""Pro and AI Plus plan prices stored in the system settings table."""
...
def plan_pricing_settings_payload() -> dict:
    """Current Pro / AI Plus monthly base and per-user prices from ``system``."""
...
def update_plan_pricing_settings(
    *,
    ...
    for amount in (pro_base_price, pro_price_per_user, ai_base_price, ai_price_per_user):
        if amount < 0:
...
def sync_subscription_plan_prices_from_system() -> bool:
    """
    Apply system-table prices to ``subscriptions`` rows for Pro and AI Plus.
    Returns True when any row was updated.
    """
    payload = plan_pricing_settings_payload()
    changed = False
    for plan_slug in PAID_PLANS:
