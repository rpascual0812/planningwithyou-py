...
def _sanitize_receipt_part(value: str) -> str:
    return _FILENAME_SAFE_RE.sub('-', (value or '').strip()).strip('-._')


def payment_receipt_filename(payment: QuotationPayment) -> str:
    """Download filename from ``transaction_id`` and ``payment_method``."""
    txn = _sanitize_receipt_part(payment.transaction_id)
    method = _sanitize_receipt_part(payment.payment_method)
    if txn and method:
        base = f'{txn}_{method}'
    else:
        base = txn or method or f'payment-{payment.pk}'
    return f'{base}.pdf'


def _receipt_company_name(payment: QuotationPayment) -> str:
    """Display name from the quotation's company (falls back to payment company)."""
    booking = payment.quotation
    company = getattr(booking, 'company', None) or payment.company
    return ((getattr(company, 'name', None) or '') or '').strip() or 'Company'
...
def _receipt_pdf_bytes(payment: QuotationPayment, receipt_number: str) -> bytes:
    booking = payment.quotation
    company_name = _receipt_company_name(payment)
    ...
    pdf.drawCentredString(
        width / 2,
        10 * mm,
        f'Generated by {company_name}. This receipt confirms successful payment capture.',
    )
...
def _payment_receipt_attachment(receipt: QuotationPaymentReceipt) -> dict[str, str | int]:
    """Structured attachment ref (resolved to PDF bytes when sending)."""
    return {'kind': 'payment_receipt', 'id': receipt.pk}


def _receipt_storage_key(payment: QuotationPayment) -> str:
    return (
        f'booking_payment_receipts/'
        f'account_{payment.account_id}/company_{payment.company_id}/'
        f'{payment_receipt_filename(payment)}'
    )


def _payment_received_context(payment: QuotationPayment) -> dict[str, str]:
    booking = payment.quotation
    contact = booking.contact
    first = (contact.first_name or '').strip() if contact else ''
    last = (contact.last_name or '').strip() if contact else ''
    company = getattr(booking, 'company', None) or payment.company
    unique_id = str(booking.unique_id or booking.pk)
    return {
        **company_template_context(company),
        ...
        'quotation_id': unique_id,
        'quotation_unique_id': unique_id,
...
def _payment_received_email_content(
    ...
    context = _payment_received_context(payment)
    if template is None:
        return (
            apply_template_placeholders(
                'Payment receipt for quotation {quotation_unique_id}',
                context,
            ),
            apply_template_placeholders(
                (
...
def ensure_paid_booking_payment_receipt(payment_id: int) -> QuotationPaymentReceipt | None:
    ...
    if not receipt.receipt_url:
        key = _receipt_storage_key(payment)
    ...
    _queue_payment_received_email(payment, receipt, use_contact_email=True)
    return receipt
...
def _payment_received_recipient(
    ...
    *,
    ...
    if use_contact_email:
        ...
        recipient = (getattr(contact, 'email', '') or '').strip() if contact else ''
        if recipient:
            return recipient
...
def _queue_payment_received_email(
    ...
    *,
    ...
    attachments = (
        [_payment_receipt_attachment(receipt)] if (receipt.storage_key or '').strip() else []
    )
